...
public class MyMap extends SurfaceView implements SurfaceHolder.Callback {
	...
	public void setBitmap(Bitmap bitmap) {
		...
		mCurrentScaleMax = mCurrentScaleMin * 15;
...
//						matrix.setTranslate((float)(CirX-object.getMapX()*15-(object.getmBitmap().getWidth()/2.0)),(float) (CirY-object.getMapY()*15-(object.getmBitmap().getHeight()/2.0)));
						if(object.getChuanshou()!=511&&object.getChuanshou()!=0) {
							matrix.preRotate((float) object.getChuanshou(), (float) object.getmBitmap().getWidth() / 2, (float) object.getmBitmap().getHeight() / 2);  //要旋转的角度
						}
