...
public class CoordinateTranslater {
  ...
  public Path pathFromPositions2( List<double[]> positions, boolean shouldClose ) {
    Path path = new Path();
    double[] start = positions.get( 0 );
    path.moveTo( (float)start[0] , (float)start[1]  );
    for( int i = 1; i < positions.size(); i++ ) {
      double[] position = positions.get( i );
      path.lineTo( (float) position[0], (float) position[1]  );
    }
    if( shouldClose ) {
      path.close();
    }
  ...
  public Path pathFromPositions3( List<double[]> positions, boolean shouldClose ) {
    Path path = new Path();
    double[] start = positions.get( 0 );
    path.moveTo( (float)start[0]*2 , (float)start[1]*2  );
    for( int i = 1; i < positions.size(); i++ ) {
      double[] position = positions.get( i );
      path.lineTo( (float) position[0]*2, (float) position[1]*2  );
    }
    if( shouldClose ) {
      path.close();
    }
